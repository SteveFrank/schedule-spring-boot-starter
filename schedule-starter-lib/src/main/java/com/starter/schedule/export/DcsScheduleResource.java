package com.starter.schedule.export;

import com.alibaba.fastjson.JSON;
import com.starter.schedule.domain.*;
...
public class DcsScheduleResource {
    ...
    private List<String> queryPathRootServerIpClazzMethod(String schedulerServerId, String ip, String clazz) throws Exception {
        return getChildren(StrUtil.joinStr(path_root, LINE, "server", LINE, schedulerServerId, LINE, "ip", LINE, ip, LINE, "clazz", LINE, clazz, LINE, "method"));
    }

    private ExecOrder queryExecOrder(String schedulerServerId, String ip, String clazz, String method) throws Exception {
        String path = StrUtil.joinStr(path_root, LINE, "server", LINE, schedulerServerId, LINE, "ip", LINE, ip, LINE, "clazz", LINE, clazz, LINE, "method", LINE, method, LINE, "value");
        if (null == client.checkExists().forPath(path)) {
            return null;
        }
    ...
    private boolean queryStatus(String schedulerServerId, String ip, String clazz, String method) throws Exception {
        String path = StrUtil.joinStr(path_root, LINE, "server", LINE, schedulerServerId, LINE, "ip", LINE, ip, LINE, "clazz", LINE, clazz, LINE, "method", LINE, method, LINE, "status");
